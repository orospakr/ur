...
#include "ur_object.h"		// class's header file
...
// class constructor
ur_object::ur_object (std::string objectPath, std::string objectName,
		      SDL_PixelFormat * screenFormat, ur_layer * hostLayer)
{
  ...
  spriteGrafx =
    new ur_sprite (objectPath + objectName + "/sprites.bmp", screenFormat);
...
ur_object::run ()
{
   ...
   * since dead things can't to much... this can be replaced with more elaborate code for funky objects...
   */
...
Sint64
ur_object::move (UR_DIRECTION_ENUM key)
{
  if (dead)
...
Sint64
ur_object::collision (ur_object * otherEntity)
{
...
Sint64
ur_object::affect_injure (Sint64 hitPoints)
{
  // weenis
  return 0;
}
Sint64
ur_object::affect_push (UR_DIRECTION_ENUM sourceDirection)
{
  // weenis
  return 0;
}
...
void
ur_object::drawToScreen (SDL_Surface * screen, SDL_Rect screenGeom)
{
