...
Object::Object(std::string objectPath, std::string objectName,
  ...
  spriteGrafx = new Sprite(objectPath + objectName + "/sprites.png", renderer);
...
void Object::run() {
   ...
   * it to nothing, since dead things can't do much... this can be replaced with
   * more elaborate code for funky objects...
   */
...
Sint64 Object::move(UR_DIRECTION_ENUM key) {
  ...
  {
    xvel = 0; // we've gone bump against the edge of the map, so full stop on
              // this axis
  ...
  {
    yvel = 0; // we've gone bump against the edge of the map, so full stop on
              // this axis
    ypos = 0;
  }
...
Sint64 Object::collision(Object *otherEntity) {
  ...
  if (faction == urFact_FF) {
    // do nothing... yet.  You nuke badniks by shooting at them and other such
    // things
  }
  return 0;
}
