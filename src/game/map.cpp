...
// class constructor
Map::Map(std::string name, SDL_Renderer *renderer, Font *fonts, Audio *audio)
    : mapName(name), screenGeom({0, 0, SCREEN_WIDTH, SCREEN_HEIGHT}),
      definition(std::make_shared<AGM>()), fontManager(fonts),
    ...
    for (int i = 0; i < definition->layers_size(); i++) {
        ...
        auto newLayer =
            new Layer(layer, this->definition, i, mapTileset, renderer);
...
void Map::loadFromAGM(std::string agmFilename) {
    ...
    if (!definition->ParseFromIstream(&input)) {
        std::cerr << "!! Unable to parse AGM file (" << agmFilename
                  << ") because: " << definition->InitializationErrorString()
                  << std::endl;
