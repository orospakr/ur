...
Audio::Audio()
    ...
    /* we're doing something a little weird here:  SDL (being a C library)
       doesn't like the callback being a member of a c++ object, so, I have made
       a little c-like routine that SDL will call. Namely, ur_SDL_audioCallback.
       Now, SDL does us a favour and lets us pass SDL a little bit of data to
       pass to the callback whenever SDL calls it.  So, it will be a pointer to
       this object and the little c-ish callback routine will be able to fone
       home.
    */
...
void Audio::load_music(std::string name) {
    ...
    loadReader.open(filename.c_str(),
    ...
    try {
        ...
        std::cout << "!! Error loading " << name << ": " << e.what()
                  << std::endl;
        return;
    }
