...
// class constructor
    Layer::Layer(const MapLayer *mapLayer, const std::shared_ptr<AGM> agm, int layerIndex, SDL_Texture *tileset,
                 SDL_Renderer *renderer) : tilepile(tileset), tilesAnimPos(0), layerIndex(layerIndex),
...
// Physics:
    ...
    Layer::checkMapPathCollision(Object *obj, Point2D position, Vector2D vector) {
        // TODO stub, for now.
        return ObjectOwner::CollisionResult{ObjectOwner::CollisionResult::None, vector, 0, nullptr};


        // step 0: make mutable copy of vector.
        // step 1: use an epsilon value to iterate along the mutable vector until we hit a solid tile. If we hit a solid tile,
        // determine which edge mutate local vector to clamp to that edge. be aware that mutated vector may cause
        // iteration issues, because length/magnitude will change.  Consider a do/while in lieu of for().
        // step 2: if no hits, return original vector with no changes.
    }
//
//    ObjectOwner::CollisionResult
//    Layer::checkMapPathCollisionOLD(Object *obj, Sint64 xpos, Sint64 ypos, Sint64 xvel,
//                                    Sint64 yvel) {
//
//        if (xvel == 0 && yvel == 0) {
...
//        );
//
//        std::cout << "Checking for collision at position: " << xpos << ", " << ypos << ", " << position << std::endl;
//
//
//        std::cout << "Velocity: " << xvel << ", " << yvel << ", " << velocity << std::endl;
//
//        // iterate by a small epsilon size to search for a tile to count as edge. Fudge (epsilon) factor 10% of tile width.
//
//
//        float epsilon = 0.02f;
//
...
//            std::cout << "  Testing point: " << testPointX << ", " << testPointY << ", " << testPoint << std::endl;
//
//            Sint64 futureBlock = this->floorGraphicalMap[testPointX][testPointY];
//
//            // TODO: CollisionResult should identify axis of collision, not assume both.
//
...
//
//                // step 2: identify collision point on that edge, and return it.
//
//
//
//
...
//                // Depending on the orientation of the vector, return the appropriate *edge* of the tile:
//                if (velocityStep.x > 0) {
//                    collisionX = static_cast<Sint64>((testPoint.x * TILE_WIDTH) - TILE_WIDTH);
//                } else if (velocityStep.x < 0) {
//                    collisionX = static_cast<Sint64>((testPoint.x * TILE_WIDTH) + TILE_WIDTH);
//                } else {
//                    collisionX = static_cast<Sint64>((testPoint.x * TILE_WIDTH));
//                }
//
//                if (velocityStep.y > 0) {
//                    collisionY = static_cast<Sint64>((testPoint.y * TILE_HEIGHT) - TILE_HEIGHT);
//                } else if (velocityStep.y < 0) {
//                    collisionY = static_cast<Sint64>((testPoint.y * TILE_HEIGHT) + TILE_HEIGHT);
//                } else {
//                    collisionY = static_cast<Sint64>((testPoint.y * TILE_HEIGHT));
//                }
//
//
//
//
////                Sint64 collisionX = static_cast<Sint64>((previousTestPoint.x * TILE_WIDTH));
////                Sint64 collisionY = static_cast<Sint64>((previousTestPoint.y * TILE_HEIGHT));
//                std::cout << "    Collision detected at point: " << collisionX << ", " << collisionY << ", "
//                          << testPoint << std::endl;
//                return ObjectOwner::CollisionResult{true, collisionX, collisionY,
...
//        // not the path. so if velocity is too high, you'll glitch through walls. Also, character stops early
//        // if velocity tile check hits solid tile, even if gap remains.
//
//        // Resolve this by searching along the vector for the first tile that is solid, and then clamping velocity
//        // to the edge of that tile.
//
//
//        // search along tile width along vector until we hit first solid tile. First, we need to determine
//        // the slope of the vector so we can determine how much displacement is required to evenly hit the same
