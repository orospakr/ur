...
Font::Font(std::string basedir, SDL_Renderer *renderer) {
    SDL_Surface *textFontSurface =
        IMG_Load((basedir + "/textfont.png").c_str());
    if (textFont == NULL)
        std::cout << "ACK!  could not text font from "
                  << basedir + "/textfont.png"
                  << " because " << SDL_GetError() << std::endl;

    // iterate through the surface and change all non-transparent pixels to
    // white.
    ...
    if (bigFont == NULL)
        std::cout << "ACK!  could not big font from "
                  << basedir + "/bigfont.png"
                  << " because " << SDL_GetError() << std::endl;
}
...
void Font::printTextToSurface(SDL_Renderer *renderer, std::string text,
    ...
    for (std::string::size_type i = 0; i < text.size(); ++i) {
        ...
        // Change the colour of the letter to the desired colour before
        // RenderCopy.

        // TODO: this has side-effect of changing color mod on the texture
